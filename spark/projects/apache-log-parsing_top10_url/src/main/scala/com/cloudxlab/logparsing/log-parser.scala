...
class Utils extends Serializable {
    ...
    def isPatternMatched(line: String): Boolean = return PATTERN.findFirstMatchIn(line).isDefined

    def extractURL(line: String): String = {
        val urlMatch = PATTERN.findFirstMatchIn(line).get
        return urlMatch.group(6)
    }

    def getTop(accessLogs:RDD[String], sc:SparkContext, topn:Int):Array[(String,Int)] = {
        // Filter invalid lines and extract only URL
        var validLogs = accessLogs.filter(isPatternMatched)
        var cleanURLs = validLogs.map(extractURL(_))
...
object EntryPoint {
    ...
    def main(args: Array[String]) {
        ...
        var results = utils.getTop(accessLogs, sc, args(1).toInt)
